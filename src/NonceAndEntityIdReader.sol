...
contract NonceAndEntityIdReader {
    /**
     * constructor
     * @param account account address
     * @param startEntityId entity id to start iterating from
     ...
     */
    constructor(IModularAccountView account, uint32 startEntityId, IEntryPoint ep, uint192 nonce) {
        ...
        while (true) {
            ...
            if (uint8(ValidationFlags.unwrap(data.validationFlags)) == 0 && data.selectors.length == 0) {
                ...
                uint256 fullNonce = ep.getNonce(address(account), nonce | (uint192(entityId) << 8));

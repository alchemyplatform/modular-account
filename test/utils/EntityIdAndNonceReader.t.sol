...
import {EntityIdAndNonceReader} from "../../src/EntityIdAndNonceReader.sol";
...
contract EntityIdAndNonceReaderTest is AccountTestBase {
    function testRead(uint152 parallelNonce, uint8 validationOptions, uint8 endEntityId) public {
        ...
        address readerFromStart = address(
            new EntityIdAndNonceReader(
                account1, entryPoint, parallelNonce << 40 | uint152(1) << 8 | validationOptions
            )
        );
        address readerFromEnd = address(
            new EntityIdAndNonceReader(
                account1, entryPoint, parallelNonce << 40 | uint152(endEntityId) << 8 | validationOptions
            )
        );

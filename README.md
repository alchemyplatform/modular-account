...
The bare-length dispatch is implemented only by `SemiModularAccount7702.isValidSignature`. Runtime validation has no bare-signature form. UserOperation and deferred-action validation add no bare-length dispatch; they continue to parse their outer signature or envelope using the existing framing and digest rules. When either path selects the native `FALLBACK_VALIDATION`, the fallback signature segment may use its existing `CONTRACT_OWNER` type, which calls ERC-1271 on the resolved fallback signer. If that signer is `address(this)`, a 64- or 65-byte bare ECDSA signature can therefore be used as the inner ERC-1271 proof. UserOperations still perform selector-applicability checks and run fallback pre-validation hooks; deferred actions still perform selector checks and reject a selected validation that has pre-validation hooks. Both paths retain the fallback enabled/signer checks and their original path-specific digest.
